...
export function CommandPalette({
  ...
  // Reset state and focus the input each time the palette opens; restore focus
  // to whatever was focused before, on close (accessibility).
  useEffect(() => {
    if (open) {
  ...
  function onKey(e: React.KeyboardEvent) {
    if (e.key === "ArrowDown") {
      ...
      // The palette is fully driven from the input (arrows + Enter). Trap focus
      // here so Tab can't move it onto a result button, which would silently
      // break arrow/Enter navigation.
      e.preventDefault();
    }
  }
        ...
        >
          ...
          >
            <div className="flex items-center gap-2 border-b border-border/60 px-4 py-3">
              ...
              />
              <kbd className="rounded bg-surface px-1.5 py-0.5 font-mono text-[10px] text-text-faint">
                ESC
              </kbd>
            </div>
            <div
              ...
              {filtered.map((cmd, i) => {
                ...
                return (
                  ...
                  >
                    <Icon
                      size={16}
                      className={cn("shrink-0", i === sel ? "text-indigo" : "text-text-faint")}
                    />
