...
import { isTauri, setTrayLabels } from "./core/ipc";
...
// System-tray menu labels per language. The tray is built natively at startup
// (before the UI language is known), so we push the localized labels down to
// Rust here. Kept out of the global i18n dictionary so they never affect the
// key-parity test.
const TRAY_LABELS: Record<
  "ru" | "en" | "fa" | "zh",
  { show: string; toggle: string; quit: string }
> = {
  ru: { show: "Показать NexusShield", toggle: "Подключить / Отключить", quit: "Выход" },
  en: { show: "Show NexusShield", toggle: "Connect / Disconnect", quit: "Quit" },
  fa: { show: "نمایش NexusShield", toggle: "اتصال / قطع", quit: "خروج" },
  zh: { show: "显示 NexusShield", toggle: "连接 / 断开", quit: "退出" },
};
...
export default function App() {
  ...
  // Keep the native tray menu labels in sync with the UI language.
  useEffect(() => {
    if (!isTauri) return;
    const l = TRAY_LABELS[language] ?? TRAY_LABELS.en;
    void setTrayLabels(l.show, l.toggle, l.quit).catch(() => {});
  }, [language]);
  ...
  if (!onboarded && servers.length === 0) {
    return (
      ...
      // OS "reduce motion" preference reactively — accessibility for users with
      // vestibular sensitivity, applied app-wide from a single place.
