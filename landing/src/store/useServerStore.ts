  ...
  persist(
    (set, get) => ({
      ...
      addFromLink: (link) => {
        ...
        set((s) =>
          s.servers.some((x) => x.id === server.id) ? s : { servers: [...s.servers, server] },
        );
        return server;
      },
      ...
      refreshSubscription: async (id) => {
        ...
        set((s) => ({
          subscriptions: s.subscriptions.map((x) =>
            // Stamp the attempt time so the scheduler's error-backoff can avoid
            // hammering a failing URL every tick.
            x.id === id ? { ...x, status: "updating", lastAttemptAt: now() } : x,
          ),
        }));
        try {
              ...
              subscriptions: s.subscriptions.map((x) =>
                x.id === id
                  ? {
                      ...x,
                      status: "ok",
                      serverCount: tagged.length,
                      lastUpdatedAt: now(),
                      lastError: undefined,
                      insecureCertAccepted,
                      usage: usage ?? x.usage,
                    }
                  : x,
              ),
