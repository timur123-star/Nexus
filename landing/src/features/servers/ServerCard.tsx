  ...
  return (
    ...
    >
      {batchMode ? (
        ...
        >
          {batchSelected ? <CheckSquare size={18} className="text-indigo" /> : <Square size={18} />}
        </button>
      ) : (
        <GripVertical
          size={15}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          {server.tags.map((tag) => (
            <span
              key={tag}
              className="shrink-0 rounded bg-surface px-1.5 text-[10px] text-text-dim"
            >
              #{tag}
            </span>
          ))}
        ...
        <div className="mt-0.5 flex items-center gap-2 text-[11px] text-text-dim">
          <span className="shrink-0 font-medium text-indigo">
      ...
      <button
        ...
        title={
          isBusy ? t("conn.connecting") : isActive ? t("common.disconnect") : t("common.connect")
        }
      >
