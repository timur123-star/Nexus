...
> = {
  en: {
    ...
    confirmBody: (name, count) =>
      `“${name}” and its ${count} server(s) will be deleted. This can't be undone.`,
    confirmYes: "Remove",
    cancel: "Cancel",
  },
  ru: {
    ...
    confirmTitle: "Удалить подписку?",
    confirmBody: (name, count) =>
      `«${name}» и её серверы (${count}) будут удалены. Это действие необратимо.`,
    confirmYes: "Удалить",
    cancel: "Отмена",
  },
  fa: {
    ...
    confirmTitle: "حذف اشتراک؟",
    confirmBody: (name, count) =>
      `«${name}» و ${count} سرور آن حذف می‌شوند. این عمل قابل بازگشت نیست.`,
    confirmYes: "حذف",
    cancel: "لغو",
  },
  zh: {
    ...
    daysLeft: (n) => `剩 ${n} 天`,
    confirmTitle: "删除订阅？",
    confirmBody: (name, count) => `“${name}”及其 ${count} 个服务器将被删除，且无法撤销。`,
    confirmYes: "删除",
    cancel: "取消",
  },
};
...
export function SubscriptionList() {
  ...
  // Subscription pending confirmation of removal (cascade-deletes its servers).
  ...
  return (
    <div className="space-y-2">
      <ConfirmDialog
        ...
        body={pendingRemove ? S.confirmBody(pendingRemove.name, pendingRemove.serverCount) : ""}
        confirmLabel={S.confirmYes}
        cancelLabel={S.cancel}
        danger
        onConfirm={() => {
          if (pendingRemove) removeSubscription(pendingRemove.id, true);
        }}
      ...
      {subscriptions.map((sub) => (
        <div key={sub.id} className="flex items-center gap-3 rounded-btn bg-bg/40 px-3 py-2.5">
          ...
          <button
            onClick={() => void refreshSubscription(sub.id).catch(() => {})}
...
function UsageRow({ sub, S }: { sub: Subscription; S: (typeof SUB_STRINGS)[Lang] }) {
  const u = sub.usage;
  if (!u) return null;
