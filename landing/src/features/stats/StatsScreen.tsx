...
export function StatsScreen() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      try {
        const next = await getConnections(clashApiPort, clashSecret);
        // The API briefly disappears during connect/disconnect; guard against a
        // late resolve after this effect was torn down.
        if (alive) setConns(next);
      } catch {
        // Transient Clash-API unavailability is expected around state changes —
        // swallow it instead of throwing an unhandled rejection every 1.5s.
        if (alive) setConns([]);
      }
    }, 1500);
    return () => {
  ...
  // tunnel the most.
  const sortedConns = [...conns].sort((a, b) => b.download + b.upload - (a.download + a.upload));
  // Scale usage bars against the single heaviest connection.
  const maxConnTotal = sortedConns.reduce((m, c) => Math.max(m, c.download + c.upload), 0);
  ...
  const avgDown = downSeries.length ? downSeries.reduce((a, b) => a + b, 0) / downSeries.length : 0;
        ...
        <div className="glass flex min-h-0 flex-col rounded-card">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-2 text-xs">
            {sortedConns.length === 0 && (
              <p className="mt-8 text-center text-text-faint">
                {xrayActive ? S.xrayNote : t("stats.noConnections")}
              </p>
            )}
            {sortedConns.map((c) => {
        ...
        <div className="glass flex min-h-0 flex-col rounded-card">
          ...
          {logTab === "core" ? (
            <div className="min-h-0 flex-1 overflow-y-auto p-2 font-mono text-[11px] leading-relaxed text-text-dim">
              {logs.length === 0 && (
                <p className="mt-8 text-center text-text-faint">{t("stats.logEmpty")}</p>
              )}
              {logs.map((l, i) => (
