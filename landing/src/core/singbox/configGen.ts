...
const MATCH_KEY: Record<Exclude<RoutingRuleMatch, "geoip" | "geosite" | "port">, string> = {
  domain: "domain",
...
function buildDns(opts: GenOptions): object {
  const servers: object[] = [
    {
      tag: "dns-remote",
      address: opts.dns.remote || "https://1.1.1.1/dns-query",
      detour: PROXY_TAG,
    },
    {
      tag: "dns-direct",
      address: opts.dns.direct || "https://223.5.5.5/dns-query",
      detour: DIRECT_TAG,
    },
  ...
  // The geosite-cn DNS rule references the `geosite-cn` rule-set, which is ONLY
  // defined in the route block when routingMode === "rule". Emitting it in
  // global/direct mode references an undefined rule-set and makes sing-box reject
  // the whole config — so gate it on the same condition.
  if (opts.routingMode === "rule") {
    rules.push({ rule_set: "geosite-cn", server: "dns-direct" });
  }

  // Fake-IP: route A/AAAA lookups to the fakeip server so domains get a synthetic
  // IP and are then routed by domain through the proxy. Requires a dedicated
  // `fakeip` DNS server entry AND a query-type rule — without BOTH, enabling the
  // `fakeip` block alone allocates a range that nothing ever resolves into, i.e.
  // fake-IP silently does nothing. This rule must come AFTER the direct rules so
  // CN/direct domains still get real IPs.
  if (opts.fakeIp) {
...
function buildRoute(opts: GenOptions): object {
  const rules: object[] = [{ action: "sniff" }, { protocol: "dns", action: "hijack-dns" }];
...
function buildOutbound(s: ServerProfile, opts: GenOptions): object {
  ...
  // xtls-rprx-vision (`flow`) is only valid over raw TCP — combined with ws /
  // grpc / h2 the core rejects the outbound. Some bad share links carry a flow
  // alongside a non-TCP transport; drop the flow in that case so the node still
  // connects (as plain VLESS) instead of failing the whole config.
  const useFlow = !!s.flow && s.transport.type === "tcp";
