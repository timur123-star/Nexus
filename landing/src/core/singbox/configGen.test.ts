...
describe("generateSingboxConfig VLESS Reality", () => {
  ...
  it("honours an explicit fingerprint when provided", () => {
    const cfg: any = generateSingboxConfig(
      server({
        tls: { enabled: true, security: "reality", publicKey: "PBK", fingerprint: "firefox" },
      }),
      baseOpts,
    );
  ...
  it("refuses post-quantum reality (no ML-DSA-65 client in sing-box)", () => {
    expect(() =>
      generateSingboxConfig(
        server({
          tls: { enabled: true, security: "reality", publicKey: "PBK", postQuantum: "PQV" },
        }),
        baseOpts,
      ),
    ).toThrow(/quantum/i);
...
describe("generateSingboxConfig DNS", () => {
  it("only references the geosite-cn rule-set when routingMode is 'rule'", () => {
    const ruleCfg: any = generateSingboxConfig(server(), {
      ...baseOpts,
      routingMode: "rule",
      fakeIp: false,
    });
    const ruleDnsTags = ruleCfg.dns.rules.map((r: any) => r.rule_set).filter(Boolean);
    expect(ruleDnsTags).toContain("geosite-cn");
    // The route block must actually DEFINE that rule-set, or the config is invalid.
    const defined = ruleCfg.route.rule_set.map((r: any) => r.tag);
    expect(defined).toContain("geosite-cn");

    // In global/direct mode the rule-set is NOT defined, so the DNS rule that
    // references it must be absent (else sing-box rejects the whole config).
    for (const mode of ["global", "direct"] as const) {
      const cfg: any = generateSingboxConfig(server(), {
        ...baseOpts,
        routingMode: mode,
        fakeIp: false,
      });
      const dnsTags = cfg.dns.rules.map((r: any) => r.rule_set).filter(Boolean);
      expect(dnsTags).not.toContain("geosite-cn");
    }
  });

  it("wires fake-IP end to end (server entry + query-type rule + fakeip block)", () => {
    const cfg: any = generateSingboxConfig(server(), { ...baseOpts, fakeIp: true });
    const serverTags = cfg.dns.servers.map((s: any) => s.tag);
    expect(serverTags).toContain("dns-fakeip");
    const fakeRule = cfg.dns.rules.find((r: any) => r.server === "dns-fakeip");
    expect(fakeRule).toBeTruthy();
    expect(fakeRule.query_type).toEqual(["A", "AAAA"]);
    expect(cfg.dns.fakeip.enabled).toBe(true);
  });

  it("omits all fake-IP machinery when disabled", () => {
    const cfg: any = generateSingboxConfig(server(), { ...baseOpts, fakeIp: false });
    expect(cfg.dns.servers.map((s: any) => s.tag)).not.toContain("dns-fakeip");
    expect(cfg.dns.rules.some((r: any) => r.server === "dns-fakeip")).toBe(false);
    expect("fakeip" in cfg.dns).toBe(false);
  });
});

describe("generateSingboxConfig flow gating", () => {
  it("drops xtls-rprx-vision flow over a non-tcp transport (ws)", () => {
    const cfg: any = generateSingboxConfig(
      server({ flow: "xtls-rprx-vision", transport: { type: "ws", path: "/v" } }),
      baseOpts,
    );
    expect("flow" in cfg.outbounds[0]).toBe(false);
  });

  it("keeps the flow over raw tcp", () => {
    const cfg: any = generateSingboxConfig(
      server({ flow: "xtls-rprx-vision", transport: { type: "tcp" } }),
      baseOpts,
    );
    expect(cfg.outbounds[0].flow).toBe("xtls-rprx-vision");
  });
});
