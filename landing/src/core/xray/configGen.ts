...
export interface XrayGenOptions {
  ...
  /** Custom DNS resolvers. Xray has no per-server detour like sing-box, so we
   *  emit a flat resolver list; absent → Xray's built-in defaults. */
  dns?: { remote?: string; direct?: string } | null;
}
...
export function generateXrayConfig(server: ServerProfile, opts: XrayGenOptions): object {
  if (
    ...
    throw new Error(`Протокол ${server.protocol} поддерживается только ядром sing-box`);
  }
  ...
  const xrayDns = buildXrayDns(opts.dns);
...
/**
 * Build an Xray `dns` object from the user's resolver settings. Xray's DNS is
 * a flat, ordered resolver list (no per-server detour like sing-box), so we feed
 * it the configured remote + direct resolvers plus `localhost` as a last resort.
 * Returns null when no custom resolvers are set so Xray uses its own defaults.
 */
function buildXrayDns(dns: XrayGenOptions["dns"]): object | null {
  if (!dns) return null;
  const servers = [dns.remote, dns.direct, "localhost"].filter(
    (s): s is string => !!s && s.trim().length > 0,
  );
  if (servers.length <= 1) return null; // only "localhost" → nothing custom
...
function buildXrayOutbound(s: ServerProfile, extra: OutboundExtras): object {
  ...
  // xtls-rprx-vision (`flow`) is only valid over raw TCP and cannot be combined
  // with mux; Xray rejects both pairings. Only honour the flow on a tcp stream,
  // and suppress mux whenever the flow is active.
  const useFlow = s.protocol === "vless" && !!s.flow && s.transport.type === "tcp";
  const muxBlock = extra.mux?.enabled && !useFlow ? { mux: { enabled: true, concurrency: 8 } } : {};
  switch (s.protocol) {
    case "vless":
      return {
        tag: PROXY_TAG,
          ...
          vnext: [
            {
              address: s.address,
              port: s.port,
              users: [{ id: s.uuid, encryption: "none", flow: useFlow ? s.flow : "" }],
            },
          ],
        },
        streamSettings,
