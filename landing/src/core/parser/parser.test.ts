...
describe("parseShareLink — shadowsocks", () => {
  ...
  it("extracts a plugin/obfs query", () => {
    const s = parseShareLink(
      "ss://aes-256-gcm:pw@1.2.3.4:8388?plugin=obfs-local%3Bobfs%3Dhttp#Obfs",
    );
...
describe("parseShareLink — hysteria2 / tuic", () => {
  it("parses hy2 alias with obfs", () => {
    const s = parseShareLink(
      "hy2://pw@h.example.com:443?sni=h.example.com&obfs=salamander&obfs-password=xyz#HY2",
    );
  ...
  it("parses tuic with uuid:password in userinfo", () => {
    const s = parseShareLink(
      "tuic://uuid-1:pass-1@t.example.com:443?congestion_control=bbr&udp_relay_mode=native#TUIC",
    );
  ...
  it("parses tuic credentials supplied via query params", () => {
    const s = parseShareLink(
      "tuic://uuid-2@t.example.com:443?password=pw2&congestion_control=cubic#TUIC2",
    );
...
describe("parseMany", () => {
  ...
  it("de-duplicates the same endpoint even with different remarks", () => {
    const text = ["trojan://p@a.com:443#First", "trojan://p@a.com:443#SecondTagDiffers"].join("\n");
  ...
  it("decodes a line-wrapped (MIME) base64 subscription body", () => {
    ...
    const wrapped = Buffer.from(inner)
      .toString("base64")
      .replace(/(.{16})/g, "$1\n");
...
describe("detectFormat", () => {
  it("classifies inputs", () => {
    ...
    expect(detectFormat(Buffer.from("trojan://a@h:1").toString("base64"))).toBe(
      "base64-subscription",
    );
...
describe("parseShareLink — wireguard / socks / hysteria(v1) / anytls", () => {
  it("parses a WireGuard link with address, reserved and mtu", () => {
    ...
    expect(s.wireguard?.localAddress).toEqual(["172.16.0.2/32", "fd01:5ca1:ab1e:80fa::1/128"]);
