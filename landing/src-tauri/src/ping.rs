...
pub fn tcp_ping(address: &str, port: u16, timeout_ms: u64) -> i64 {
    ...
    // Bound the TOTAL probe time to `timeout`, not `timeout × number-of-IPs`.
    // A host resolving to several dead addresses would otherwise block a worker
    // thread for many seconds (each `connect_timeout` is per-address).
    ...
    for addr in addr_iter {
        ...
        if std::net::TcpStream::connect_timeout(&addr, remaining).is_ok() {
