...
pub fn run() {
    // Safety net of last resort. If the process panics anywhere — including in a
    // supervisor or pipe thread — the kill-switch may be armed (default-deny
    // outbound = NO internet) and/or the system proxy set. `panic = "abort"`
    // still runs the panic hook before aborting, so disarm both here so a crash
    // can never strand the user offline. Best-effort; both are idempotent and
    // only touch our own tagged rules.
