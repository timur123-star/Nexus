...
pub fn core_start(
    ...
    // Reset the traffic-rate baseline so the first poll of the NEW core doesn't
    // compute a bogus delta against the previous (now-dead) core's counters.
...
/// enrolment), so we seed a tiny splitmix64 from the current nanos mixed with
/// the process id and a monotonic per-call counter — guaranteeing distinct ids
/// even for back-to-back calls within the same nanosecond, without relying on
/// undefined-behaviour tricks like reading a stack address.
fn random_hex_22() -> String {
    ...
    let mut x = nanos ^ seq.wrapping_mul(0x9E37_79B9_7F4A_7C15) ^ pid.rotate_left(32);
...
pub async fn get_traffic(
    ...
    // A failed request OR an unparseable body both mean "core not up / not ready
    // yet" — return zeros rather than surfacing a hard error to the UI, which
    // would otherwise flap an error toast during every connect/disconnect.
    let body: Value = match req.send().await {
        Ok(r) => match r.json().await {
            Ok(v) => v,
            Err(_) => return Ok(TrafficStats::default()),
        },
        Err(_) => return Ok(TrafficStats::default()),
    };
    ...
    if let Some(conns) = body["connections"].as_array() {
        for c in conns {
            ...
            out.push(ConnectionEntry {
                ...
                // Clash API reports `start` as an RFC3339 timestamp; convert to
                // epoch millis so the UI can show connection age. 0 if absent.
                start: c["start"]
                    .as_str()
...
/// Localize the system-tray menu labels. The frontend calls this on startup and
/// whenever the UI language changes, passing already-translated strings so the
/// tray matches the rest of the app instead of being hardcoded to one language.
#[tauri::command]
pub fn set_tray_labels(app: AppHandle, show: String, toggle: String, quit: String) {
    crate::tray::set_labels(&app, &show, &toggle, &quit);
}
