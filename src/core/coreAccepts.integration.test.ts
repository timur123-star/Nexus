  ...
  outbounds: [
    { type: "selector", tag: "select", outbounds: ["auto"] },
    {
      ...
      tls: {
        ...
        reality: { enabled: true, public_key: "mL8S_P3Szjj-uDI32836ntWvHTJDu52Q-uMLnsBuyAU", short_id: "0123abcd" },
      },
    },
    {
      type: "trojan",
...
describe.skipIf(!haveSingbox)("sing-box accepts every supported protocol", () => {
  const cases: Array<[string, ServerProfile]> = [
    ["vmess-ws", srv({ protocol: "vmess", uuid: "11112222-3333-4444-5555-666677778888", transport: { type: "ws", path: "/ws", host: "h.com" }, tls: { enabled: true, security: "tls", sni: "h.com" } })],
    ["tuic", srv({ protocol: "tuic", uuid: "11112222-3333-4444-5555-666677778888", password: "pw", transport: { type: "quic" }, tls: { enabled: true, security: "tls", sni: "h" } })],
    ...
    ["hysteria-no-bw", srv({ protocol: "hysteria", password: "pw", transport: { type: "quic" }, tls: { enabled: true, security: "tls", sni: "h" } })],
    ["shadowtls", srv({ protocol: "shadowtls", transport: { type: "tcp" }, tls: { enabled: true, security: "tls", sni: "www.microsoft.com" }, shadowtls: { version: 3, password: "hs", method: "aes-128-gcm", ssPassword: "sspw" } })],
    ["socks", srv({ protocol: "socks", port: 1080, transport: { type: "tcp" }, tls: { enabled: false, security: "none" } })],
    ["http", srv({ protocol: "http", port: 8080, transport: { type: "tcp" }, tls: { enabled: false, security: "none" } })],
  ];
