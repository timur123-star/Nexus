...
const vmess = "vmess://" + b64(JSON.stringify({
  v: "2", ps: "VMess WS TLS", add: "v.example.com", port: "443", id: "b831381d-6324-4d53-ad4f-8cda48b30811",
  aid: "0", scy: "auto", net: "ws", type: "none", host: "cdn.example.com", path: "/vm", tls: "tls", sni: "cdn.example.com",
}));
const ssSip002 = "ss://" + b64("aes-256-gcm:hunter2") + "@ss.example.com:8388#SS%20SIP002";
const ssLegacy = "ss://" + b64("chacha20-ietf-poly1305:p%40ss@ss2.example.com:443") + "#SS%20Legacy";
const ssPlugin = "ss://" + b64("aes-128-gcm:secret") + "@ss3.example.com:8388?plugin=obfs-local%3Bobfs%3Dhttp%3Bobfs-host%3Dwww.bing.com#SS%20obfs";

const LINKS: Record<string, string> = {
  "vless-ws-tls": "vless://uuid-aaaa-bbbb@v.example.com:443?type=ws&security=tls&sni=ex.com&host=cdn.com&path=%2Fws&flow=&encryption=none#VLESS%20WS",
  "vless-reality": "vless://uuid-cccc-dddd@r.example.com:443?type=tcp&security=reality&pbk=PUBKEY123&sid=ab12&fp=chrome&sni=www.microsoft.com&flow=xtls-rprx-vision#VLESS%20REALITY",
  "vless-xhttp": "vless://uuid-eeee@x.example.com:443?type=xhttp&security=tls&path=%2Fxh&host=h.com&sni=h.com&mode=auto#XHTTP",
  vmess,
  "trojan-grpc": "trojan://p%40ssw0rd@t.example.com:443?type=grpc&serviceName=grpcsvc&security=tls&sni=t.example.com#Trojan%20gRPC",
  "ss-sip002": ssSip002,
  "ss-legacy": ssLegacy,
  "ss-plugin": ssPlugin,
  hysteria2: "hysteria2://pa%24%24@h2.example.com:443?sni=h2.example.com&obfs=salamander&obfs-password=zzz&insecure=1#Hy2",
  "hy2-alias": "hy2://auth123@h2b.example.com:8443?sni=h2b.example.com#Hy2%20alias",
  hysteria1: "hysteria://h1.example.com:36712?auth=secret&peer=h1.example.com&insecure=1&upmbps=50&downmbps=200&obfs=xplus#Hy1",
  ...
  wireguard: "wireguard://cPrivKeyBase64%3D@wg.example.com:51820?publickey=PeerPubKey%3D&presharedkey=PSK%3D&address=172.16.0.2%2F32,fd01::2%2F128&reserved=0,0,0&mtu=1420#WG",
  ...
  shadowtls: "shadowtls://aes-256-gcm:innerpass@st.example.com:443?password=handshakepw&version=3&sni=www.apple.com#ShadowTLS",
  ssh: "ssh://root:rootpw@ssh.example.com:22#SSH",
  "ssh-key": "ssh://deploy@ssh2.example.com:2222?privateKey=" + b64url("-----BEGIN OPENSSH PRIVATE KEY-----\nabc\n-----END OPENSSH PRIVATE KEY-----") + "&passphrase=pp#SSH%20key",
  tor: "tor://#Tor",
...
describe("subscription import stress audit", () => {
  ...
  it("parses a plain newline link-list with comments & blank lines", () => {
    const body = [
      "# my subscription", "", LINKS["vless-ws-tls"], "  ", "// note", LINKS.vmess, LINKS["ss-sip002"], LINKS.tuic,
    ].join("\n");
  ...
  it("dedups identical endpoints but keeps CDN variants distinct", () => {
    ...
    const variant = LINKS["vless-ws-tls"].replace("%2Fws", "%2Fother").replace("cdn.com", "cdn2.com");
  ...
  it("does NOT misparse a real subscription URL as an http proxy", () => {
    ...
    expect(() => parseShareLink("https://panel.example.com/api/v1/client/subscribe?token=x")).toThrow();
  });
});
