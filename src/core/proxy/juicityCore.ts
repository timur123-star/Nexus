 ...
 * protocol. It exposes a local HTTP+SOCKS listener (no TUN, no Clash API), so
 * the connection store routes a `juicity` server here and the Rust supervisor
 * falls back to its no-API readiness path.
 */
