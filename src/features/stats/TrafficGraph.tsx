...
export function TrafficGraph({
  ...
  const idlePath = useMemo(() => (hasData ? "" : toPath(idleWave().map((v) => v * max), max)), [hasData, max]);
  ...
  return (
    <div className="flex gap-3">
      {/* Y-axis labels */}
      <div className="flex flex-col justify-between py-0.5 text-right text-[10px] font-mono text-text-faint" style={{ height: 150 }}>
        {labels.map((l, i) => (
          <span key={i} className="whitespace-nowrap leading-none">{l}</span>
        ))}
      </div>
      ...
      <div className="relative flex-1" style={{ height: 150 }}>
        ...
        >
          ...
          {downPath && (
            <path d={downPath} fill="none" stroke="var(--color-teal)" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />
          )}
          {upPath && (
            <path d={upPath} fill="none" stroke="var(--color-indigo)" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />
          )}
        </svg>
      </div>
    </div>
