...
const STATS_STRINGS: Record<
  Lang,
  { session: string; uptime: string; peak: string; avg: string; idle: string; noStatsNote: string }
> = {
  en: {
    ...
    idle: "Not connected",
    noStatsNote:
      "Live traffic and connection stats need the Clash API, which the {core} core does not expose — these counters stay at zero. The logs below still work.",
  },
  ru: {
    ...
    noStatsNote:
      "Статистика трафика и соединений работает через Clash API, которого нет у ядра {core} — счётчики остаются на нуле. Логи ниже работают.",
  },
  fa: {
    session: "\u0646\u0634\u0633\u062a \u0641\u0639\u0644\u06cc",
    uptime: "\u0632\u0645\u0627\u0646 \u0641\u0639\u0627\u0644\u06cc",
    ...
    idle: "\u0645\u062a\u0635\u0644 \u0646\u06cc\u0633\u062a",
    noStatsNote:
      "آمار ترافیک و اتصال‌ها به Clash API نیاز دارد که هسته {core} آن را ارائه نمی‌دهد؛ شمارنده‌ها صفر می‌مانند. گزارش‌های زیر کار می‌کنند.",
  },
  zh: {
    session: "\u5f53\u524d\u4f1a\u8bdd",
    uptime: "\u8fd0\u884c\u65f6\u95f4",
    ...
    idle: "\u672a\u8fde\u63a5",
    noStatsNote:
      "流量和连接统计需要 Clash API，而 {core} 内核不提供该接口——此处计数器保持为零。下方日志仍可用。",
  },
};
...
export function StatsScreen() {
  ...
  // Live traffic/connection stats come from the Clash API, which only sing-box
  // exposes. For any other active core they stay at zero — say so honestly.
  const noLiveStats = connected && activeCore != null && !getCore(activeCore).providesClashApi;
  const noStatsNote = activeCore
    ? S.noStatsNote.replace("{core}", getCore(activeCore).label)
    : S.noStatsNote;
  ...
  // is actually running.
  useEffect(() => {
    // Only sing-box answers the Clash API; skip polling (and clear) otherwise.
    if (status !== "connected" || !activeCore || !getCore(activeCore).providesClashApi) {
      setConns([]);
      return;
    }
    ...
    return () => clearInterval(id);
  }, [status, activeCore, clashApiPort, clashSecret]);
  ...
  const coreLabel = activeCore ? getCore(activeCore).label : null;
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-5xl flex-col gap-4 p-5">
      ...
      {/* Cores without a Clash API (Xray / Juicity / Naïve) can't feed live
          counters — be honest about the zeros instead of silently showing them. */}
      {noLiveStats && (
        ...
        <div className="glass flex min-h-0 flex-col rounded-card">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-2 text-xs">
            {sortedConns.length === 0 && (
              <p className="mt-8 text-center text-text-faint">{noLiveStats ? noStatsNote : t("stats.noConnections")}</p>
            )}
            {sortedConns.map((c) => {
