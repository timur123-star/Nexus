...
vi.mock("../core/ipc", () => ({
  fetchSubscriptionInfo: (url: string, insecure: boolean, ua: string) => fetchInfo(url, insecure, ua),
  pingServer: vi.fn(),
}));
// Avoid pulling the real persistence/db layer into the node test env.
vi.mock("../core/db", () => ({ persistentStorage: { getItem: vi.fn(), setItem: vi.fn(), removeItem: vi.fn() } }));
...
describe("fetchSubscriptionResilient", () => {
  ...
  it("retries insecure when the native error names a certificate problem", async () => {
    fetchInfo
      .mockRejectedValueOnce(new Error("error sending request for url (https://h/x): invalid peer certificate: UnknownIssuer"))
  ...
  it("retries insecure for a bare-IP https panel even when the error is opaque", async () => {
    ...
    fetchInfo
      .mockRejectedValueOnce(new Error("error sending request for url (https://195.35.24.251/sub/i3)"))
      .mockResolvedValueOnce(PAYLOAD);
