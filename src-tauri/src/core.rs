...
//! the `sing-box` binary, spawn it, and stream its stdout/stderr back to the UI
//! as `core://log` events. Status transitions are emitted on `core://status`.
...
impl CoreManager {
    ...
    /// Write the config and (re)start the core.
    pub fn start(&mut self, app: &AppHandle, config_json: &str) -> Result<(), String> {
        ...
        log::info!("sing-box started ({})", bin.display());
        Ok(())
    }
...
/// Resolve the sing-box executable. Search order:
///   1. bundled resource  <resources>/binaries/sing-box[.exe]
///   2. next to our own exe
///   3. PATH (bare "sing-box")
fn locate_singbox(app: &AppHandle) -> Result<PathBuf, String> {
